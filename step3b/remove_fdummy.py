...
def FDUMMY_adjust(entry):
 """ Return a list of entries.
  If entry has one or more 'FDUMMY' groups, 
   remove them from entry
  return adjusted entry (as a singleton list of entries)
 """
 dbg = False
 if 'FDUMMY' not in entry.gtypes:
  return False,[entry]  # no change
 ...
 for i,gtype in enumerate(entry.gtypes):
  ...
  if gtype == 'FDUMMY':
   # nothing else to do. this group not part of newgroups
   pass
  else:
...
if __name__=="__main__":
 ...
 for entry0 in entries0:
  flag,newentries = FDUMMY_adjust(entry0)
  if flag:
   nadj = nadj + 1
  for entry in newentries:
   entries.append(entry)
 print(nadj,"entries adjusted for FDUMMY")
